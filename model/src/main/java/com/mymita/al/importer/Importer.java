...
public class Importer {
	...
	private void importData(final ClassPathXmlApplicationContext ctx)
			throws BeansException, IOException {
		ctx.getBean(ImportService.class).importPersons(
				"00 Personen Abfrage.txt",
				new CountingImportListener<Object>() {

					@Override
					public void onImport(final Object object) {
						super.onImport(object);
						System.out.println(String.format("[%d/%d] Imported %s",
								count(object), max(object), object));
					}
				});
	}
