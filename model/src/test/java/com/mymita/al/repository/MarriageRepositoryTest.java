...
public class MarriageRepositoryTest extends AbstractTransactionalTestNGSpringContextTests {
  ...
  public void findByLastNameOrBirthNameAndYearOfDeathNeg() throws Exception {
    assertThat(
        marriageRepository.findAll(or(
            marriage.lastNamePerson1.containsIgnoreCase("höh"),
            marriage.birthNamePerson2.containsIgnoreCase("")).and(marriage.year.eq("1900"))),
        Matchers.<Marriage> iterableWithSize(0));
  }
  ...
  public void findByLastNameOrBirthNameAndYearOfDeathPos() throws Exception {
    assertThat(
        marriageRepository.findAll(or(
            marriage.lastNamePerson1.containsIgnoreCase("höh"),
            marriage.birthNamePerson2.containsIgnoreCase("")).and(marriage.year.eq("2010"))),
        Matchers.<Marriage> iterableWithSize(1));
  }
